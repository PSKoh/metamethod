...
-   The `tradmeta` data frame is reordered to prepare it for visualization. The `order()` function sorts the rows first by `Creativity.Measure_type` (grouping same creativity measures together) and then by `yi` (the effect sizes, from smallest to largest within each measure type). Do note that the comma before ']' is required as it tells R to keep all columns while reordering the rows according to the specified order.
...
For reference, the funnel plot will appear as shown on page 8 of this handbook.
...
For reference, the forest plot of moderators will appear as shown on page 16 of this handbook.

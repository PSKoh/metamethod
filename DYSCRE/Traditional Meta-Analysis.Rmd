...
-   The `tradmeta$sei_corrected` variable is created to store the corrected standard error for each effect size, calculated using the formula $\sqrt{\frac{(n_{dys} + n_{control})}{(n_{dys} \cdot n_{control})}}$. Standard error is corrected in this case to account for unequal sample sizes

...
-   The `n1i` and `n2i` functions specify the columns for the sample sizes of each group

-   The `m1i` and `m2i` functions specify the columns for the means of each group.

-   The `sd1i` and `sd2i` functions specify the columns for the standard deviations of each group.
...
-   The `mlmmetaresults` object contains the results of the meta-analysis, and the `ranktest()` function uses this object to perform the rank correlation test.
...
-   The `multilevelmeta$sei_corrected` variable is created to store the corrected standard error for each effect size, calculated using the formula \(\sqrt{\frac{(n_{p} + n_{a})}{(n_{p} \cdot n_{a})}}\).
    Using Egger' test (unadjusted) on SMDs results in inflated type 1 error as SMD and SE are not independent. Hence, use corrected formula for SE.
...
-   The `lmer()` function is used to fit a linear mixed-effects model, where the effect size weighted by the corrected standard error is predicted by the intercept and the inverse of the corrected standard error.
    metafor::rma.mv does not have a weights argument, and metafor::regtest does not support rma.mv objects.
...
Note that if convergence issues arise, the `control` function can be used to adjust the convergence criteria.
...
-   The `rows` function specifies the arrangement of studies, with the order of rows adjusted to group studies by publication type.

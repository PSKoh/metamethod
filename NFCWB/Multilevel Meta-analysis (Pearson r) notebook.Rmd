...
-   The `mlmmeta` data frame is reordered to prepare it for visualization. The `order()` function sorts the rows first by publication_type (grouping the same publication types together) and then by `yi` (arranging effect sizes from smallest to largest within each publication type). Do note that the comma before ] is required as it tells R to keep all columns while reordering only the rows according to the specified order.
...
For reference, the forest plot will appear as shown on page 7 of this handbook.
...
```{r, fig.width = 13, fig.height = 30, echo=FALSE}
...
For reference, the funnel plot will appear as shown on page 10 of this handbook.
...
For reference, the forest plot of moderators will appear as shown on page 17 of this handbook.
